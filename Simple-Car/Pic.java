...
//clipX=0;  clipY=0;  clipW=numCols;  clipH=numRows;
  ...
  public static void main(String[] args)
  {
   ...
   // activate this texture in texture unit 0
   public void activate() {

      GL13.glActiveTexture( GL13.GL_TEXTURE0 );
           Util.error("after activate texture 0");

      if ( textureId == -1 ) {// generate once per pic
         setTextureId( GL11.glGenTextures() );
         System.out.println("generated texture name is " + getTextureId() );
                Util.error("after generate texture id " + getTextureId() );
      }
...
System.out.println("Pic is searching for [" + s + "] in static list");
     for (int k=0; k<list.size(); k++ ) {
        if ( list.get(k).name.equals( s ) ) {
           System.out.println("found it in position " + k);
           return k;
        }
     }
     return -1;
